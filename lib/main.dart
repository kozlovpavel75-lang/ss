...
class _MainScreenState extends State<MainScreen> with SingleTickerProviderStateMixin {
      ...
      actions: [
        ...
        IconButton(
          icon: Icon(
            ...
            color: isKyivMode.value ? const Color(0xFFE8D98C) : Colors.white38,
            size: 22,
          ),
  ...
  // Київ-фон — темно-синє небо з зірками (без зовнішніх фото)
  ...
  Widget _buildHome() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // ── Метрики ──
        Row(children: [
          _metricCard('ЗАПИСІВ',    total.toString(),   AppColors.uaBlue),
          const SizedBox(width: 10),
          _metricCard('ОБРАНИХ',    favCnt.toString(),  AppColors.uaYellow),
          const SizedBox(width: 10),
          _metricCard('ДОКУМЕНТІВ', docs.length.toString(), AppColors.accent),
        ]),
              ...
              children: logs.take(5).map((log) => Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(log,
                      style: const TextStyle(fontSize: 10, color: Colors.greenAccent, fontFamily: 'JetBrainsMono'),
    ...
    final card = Card(
          ...
          child: ListTile(
            ...
            subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(p.content, maxLines: 1, overflow: TextOverflow.ellipsis,
              ...
              if (p.variables.isNotEmpty) Padding(
                ...
                child: Wrap(spacing: 4, children: p.variables.take(4).map((v) => Container(
                  ...
                  decoration: BoxDecoration(
                    color: catColor(cat).withOpacity(0.1),
                    borderRadius: BorderRadius.circular(4),
                    border: Border.all(color: catColor(cat).withOpacity(0.3)),
                  ),
                  child: Text('{$v}', style: TextStyle(
                      fontSize: 9, color: catColor(cat), fontFamily: 'JetBrainsMono')),
                )).toList()),
              ),
            ]),
            onTap: () {
